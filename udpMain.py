    ...
    while True:
        try:
            if sensorgateway.ip != '':
                ...
                print(gendate() + ':暂未获取到有效的网关IP,本次温湿度查询跳过！')
        except:
            print(gendate() + ':请求环境温度发送失败')
        finally:
            await asyncio.sleep(sensorgateway.interval)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(gendate() + ': MQTT服务器连接失败->' + str(e))
        while mqttconnectflag == False:
            print(gendate() + ': 尝试重新连接MQTT服务器...')
            try:
                mqclient.connect(mqttobj.host, mqttobj.port, mqttobj.keepalive)
                mqttconnectflag = True
            except Exception as e:
                print(gendate() + ': MQTT服务器重连失败->' + str(e))
                mqttconnectflag = False
            finally:
                time.sleep(5)
